...
<head>
	<% 
		...
		if (request.getParameter("showUserID") != null) {
			showUserID = request.getParameter("showUserID");
		}
		if (showUserID == null) {
		...
		UserDTO showUserDTO = userDAO.getUser(showUserID);
		...
		boardList = boardDAO.getListInMypage(search, showUserID);
...
<div class = "jumbotron" style="margin-top:62px;padding-bottom: 60px;">
			<h2 class = "text-center"><img class="rounded-circle" src="<%=userDAO.getProfile(showUserID)%>" style="width:100px;height:100px;"></h2>
			<p class = "text-center">
				<font color="white"><%=showUserDTO.getUserNickname()%>(<%=showUserDTO.getUserID()%>)<br>
					<%=showUserDTO.getUserIntro()%></font></p>
			<form name="searchForm" method="get" action="mypage.jsp">
				<input type="hidden"name="showUserID" value="<%=showUserID%>">
				<input type="hidden" id="search" name="search" value="">
			<p class = "text-center">
				<button class="btn btn-default" type="button" onclick="$('#search').val('활동');searchForm.submit();">활동<strong><%=boardDAO.getBoardCnt(showUserID,"mypage")%></strong></button>
				<button class="btn btn-default" type="button" onclick="$('#search').val('좋아요');searchForm.submit();">좋아요<strong><%=likeDAO.getLikeCnt(showUserID)%></strong></button>
			</p>
			</form>
		</div>
