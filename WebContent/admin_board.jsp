		...
		<table class="table" style="text-align: center;border: 1px solid #dddddd">
			<thead>
				<tr>
					...
					<th style="background-color: #fafafa;text-align: center;">삭제여부</th>
			...
			for (var i = 0; i < result.length; i++) {
				...
				for (var j = 0; j < result[i].length; j++) {
					...
					if (j == result[i].length - 1) {
						...
						if(result[i][4].value == 'N'){
						cell.innerHTML = "<button class='btn btn-default' type='button' onclick='deleteFunction(\""
								+ result[i][0].value + "\")'>삭제</button>";
						}
					}
				}
			}
